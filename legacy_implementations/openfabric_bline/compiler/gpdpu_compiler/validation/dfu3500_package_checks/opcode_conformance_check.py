"""DFU3500 active instruction opcode metadata conformance checks."""
...
DFU3500_OPCODE_CONFORMANCE_SPEC = CheckSpec(
    name="dfu3500_opcode_conformance",
...
def run_dfu3500_opcode_conformance_check(
    artifact_root: Path,
    ...
    *,
    ...
    if profile.target != "dfu3500":
        issues.append(
            _issue(
                "dfu3500_opcode_conformance_profile_mismatch",
                "DFU3500 opcode conformance check requires a dfu3500 profile.",
                None,
            )
        )
    ...
    if cbuf is not None and micc is not None and not issues:
        _check_active_opcode_metadata(cbuf, micc, profile, issues)

    status = "fail" if issues else "pass"
    return ValidationReport(
        schema_version="dfu_validation_check_report_v1",
        check_name=DFU3500_OPCODE_CONFORMANCE_SPEC.name,
        status=status,
        authoritative=DFU3500_OPCODE_CONFORMANCE_SPEC.applies_to_gate(requested_gate),
        ...
        policy={
            "mode": "strict",
            "source": "dfu3500_active_cbuf_rows_vs_registerOp_metadata",
            "checked_fields": ("opCode", "unit_inst_type", "latency"),
        },
        issues=tuple(issues),
    )


def _check_active_opcode_metadata(
    cbuf: bytes,
    micc: bytes,
    profile: DfuBinaryProfile,
    issues: list[ValidationIssue],
...
def _check_block_opcode_metadata(
    cbuf: bytes,
    micc: bytes,
    task_id: int,
    subtask_id: int,
...
def _check_inst_opcode_metadata(
    row: bytes,
    task_id: int,
    subtask_id: int,
    block_row: int,
    ...
    unit_inst_type = struct.unpack_from("<Q", row, 8)[0]
    latency = struct.unpack_from("<Q", row, 16)[0]
    ...
    if annotation.get("pseudo"):
        issues.append(
            _issue(
                "dfu3500_opcode_pseudo_in_cbuf",
                (
                    f"Active stage {stage_index} of ({task_id}, {subtask_id}) "
                    f"block row {block_row} points to assembler-only opcode "
                    f"{annotation['mnemonic']} at {path}."
                ),
                f"{path}.opCode",
                details={
                    **issue_details,
    ...
    expected_unit_inst_type = annotation.get("unit_inst_type")
    if expected_unit_inst_type is not None and unit_inst_type != expected_unit_inst_type:
        issues.append(
            _issue(
                "dfu3500_opcode_unit_type_mismatch",
                (
                    f"Active CBUF row {path} has unit_inst_type {unit_inst_type}, "
                    f"expected {expected_unit_inst_type} for {annotation['mnemonic']}."
                ),
                f"{path}.unit_inst_type",
                details={
                    **issue_details,
                    "actual_unit_inst_type": unit_inst_type,
                    "expected_unit_inst_type": expected_unit_inst_type,
                },
            )
        )
    expected_latency = annotation.get("latency")
    if expected_latency is not None and latency != expected_latency:
        issues.append(
            _issue(
                "dfu3500_opcode_latency_mismatch",
                (
                    f"Active CBUF row {path} has latency {latency}, "
                    f"expected {expected_latency} for {annotation['mnemonic']}."
                ),
                f"{path}.latency",
                details={
                    **issue_details,
