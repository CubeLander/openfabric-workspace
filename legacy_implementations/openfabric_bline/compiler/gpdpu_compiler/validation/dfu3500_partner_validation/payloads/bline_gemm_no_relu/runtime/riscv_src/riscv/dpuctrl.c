...
unsigned long pe_cycle;

void DMA_Transfer_input(uint64_t ddr_start_addr0,
						uint64_t ddr_start_addr1,
	...
	if (!regular_mark) {
		dma_transfer_conf conf_mem2spm_a = { 0 };
		conf_mem2spm_a.MemAddr_Ch0 = (void*)&ddr_start_addr0;
		conf_mem2spm_a.SpmAddr0 = (void*)spm_start_addr0;
		...
		conf_mem2spm_a.MemAddr_Ch1 = (void*)&ddr_start_addr1;
		conf_mem2spm_a.SpmAddr1 = (void*)spm_start_addr1;
...
void DMA_Transfer_output(uint64_t ddr_start_addr0,
						uint64_t ddr_start_addr1,
						unsigned spm_start_addr0,
						unsigned spm_start_addr1,
						unsigned x_slice,
	...
	if (!output_regular_mark) {
		dma_transfer_conf conf_spm2mem_c = { 0 };
		conf_spm2mem_c.MemAddr_Ch0 = (void*)&ddr_start_addr0;
		conf_spm2mem_c.SpmAddr0 = (void*)spm_start_addr0;
		...
		conf_spm2mem_c.MemAddr_Ch1 = (void*)&ddr_start_addr1;
		conf_spm2mem_c.SpmAddr1 = (void*)spm_start_addr1;
...
int do_dpuctrl(int argc, char *argv[])
{
	printf("---------------- ------\n");
	printf("enter gemm_templete\n");
	__raw_writel(0x0, MICC_SIMD_MODE_CHOICE);
	...
	//transfer inst
	__raw_writel(0x1,MICC_CYCLE_COUNT0);
		DPU_CbufTransfer((void*)&inst_in_mem);
		while (!(DPU_DMATransferFinish(2)));
	pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
	__raw_writel(0x2,MICC_CYCLE_COUNT0);
	printf("cbuf cycle all= %ld\n",pe_cycle);
...
int i;
	if (app_M != 1 || app_N != 1) {
		...
		for (i = 0; i < app_M * app_N * app_K; i++) {
			...
			if (app_M != 1 && app_K != 1) {
				for (int batch = 0; batch < INPUT_BATCH_SIZE / app_batch; batch++) {
					...
					__raw_writel(0x1,MICC_CYCLE_COUNT0);
					DMA_Transfer_input(
						MEM_GEMM_INPUT1_ADDR + GEMM_INPUT1_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR + ddr_batch,
						MEM_GEMM_INPUT1_ADDR + GEMM_INPUT1_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR + ddr_batch,
				GEMM_INPUT1_spmStartAddr[app_num][0] + spm_batch, GEMM_INPUT1_spmStartAddr[app_num][1] + spm_batch,
					...
					pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
					__raw_writel(0x2,MICC_CYCLE_COUNT0);
					printf("APP%d batch%d GEMM_INPUT1 cycle all= %ld\n",app_num, batch, pe_cycle);
				}
			} else {
				__raw_writel(0x1,MICC_CYCLE_COUNT0);
				DMA_Transfer_input(
					MEM_GEMM_INPUT1_ADDR + GEMM_INPUT1_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR,
					MEM_GEMM_INPUT1_ADDR + GEMM_INPUT1_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR,
				...
				pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
				__raw_writel(0x2,MICC_CYCLE_COUNT0);
				printf("APP%d GEMM_INPUT1 cycle all= %ld\n",app_num, pe_cycle);
			}
			...
			if (app_K != 1 && app_N != 1) {
				for (int batch = 0; batch < INPUT_BATCH_SIZE / app_batch; batch++) {
					...
					__raw_writel(0x1,MICC_CYCLE_COUNT0);
					DMA_Transfer_input(
						MEM_GEMM_INPUT2_ADDR + GEMM_INPUT2_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR + ddr_batch,
						MEM_GEMM_INPUT2_ADDR + GEMM_INPUT2_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR + ddr_batch,
				GEMM_INPUT2_spmStartAddr[app_num][0] + spm_batch,  GEMM_INPUT2_spmStartAddr[app_num][1] + spm_batch,
					...
					pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
					__raw_writel(0x2,MICC_CYCLE_COUNT0);
					printf("APP%d batch%d GEMM_INPUT2 cycle all= %ld\n",app_num, batch, pe_cycle);
				}
			} else {
				__raw_writel(0x1,MICC_CYCLE_COUNT0);
				DMA_Transfer_input(
					MEM_GEMM_INPUT2_ADDR + GEMM_INPUT2_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR,
					MEM_GEMM_INPUT2_ADDR + GEMM_INPUT2_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR,
				...
				pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
				__raw_writel(0x2,MICC_CYCLE_COUNT0);
				printf("APP%d GEMM_INPUT2 cycle all= %ld\n",app_num, pe_cycle);
			}

			if (i == 0 || GEMM_INPUT3_ddrStartAddr_rtl[app_num][0] != b * (INPUT_BATCH_SIZE / app_batch) * GEMM_INPUT3_HEIGHT * GEMM_INPUT3_WIDTH * sizeof(short)) {
			if (app_M != 1 && app_N != 1) {
				for (int batch = 0; batch < INPUT_BATCH_SIZE / app_batch; batch++) {
					...
					__raw_writel(0x1,MICC_CYCLE_COUNT0);
					DMA_Transfer_input(
						MEM_GEMM_INPUT3_ADDR + GEMM_INPUT3_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR + ddr_batch,
						MEM_GEMM_INPUT3_ADDR + GEMM_INPUT3_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR + ddr_batch,
				GEMM_INPUT3_spmStartAddr[app_num][0] + spm_batch,  GEMM_INPUT3_spmStartAddr[app_num][1] + spm_batch,
					...
					pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
					__raw_writel(0x2,MICC_CYCLE_COUNT0);
					printf("APP%d batch%d GEMM_INPUT3 cycle all= %ld\n",app_num, batch, pe_cycle);
				}
			} else {
				__raw_writel(0x1,MICC_CYCLE_COUNT0);
				DMA_Transfer_input(
					MEM_GEMM_INPUT3_ADDR + GEMM_INPUT3_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR,
					MEM_GEMM_INPUT3_ADDR + GEMM_INPUT3_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR,
				...
				pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
				__raw_writel(0x2,MICC_CYCLE_COUNT0);
				printf("APP%d GEMM_INPUT3 cycle all= %ld\n",app_num, pe_cycle);
			}
			}
			...
			DPU_Kernel_Start(inst_reload, TASK_NUM, (void*)(((app_num % 2) * 0x400000) / 4), (app_num % 2), 0);
			...
			if (app_M != 1 && app_N != 1) {
				for (int batch = 0; batch < INPUT_BATCH_SIZE / app_batch; batch++) {
					...
					__raw_writel(0x1,MICC_CYCLE_COUNT0);
					DMA_Transfer_output(
					MEM_GEMM_OUTPUT1_ADDR + GEMM_OUTPUT1_ddrStartAddr_rtl[app_num][0] + SPM_RST_DDR_ADDR + ddr_batch,
					MEM_GEMM_OUTPUT1_ADDR + GEMM_OUTPUT1_ddrStartAddr_rtl[app_num][1] + SPM_RST_DDR_ADDR + ddr_batch,
					...
					pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
					__raw_writel(0x2,MICC_CYCLE_COUNT0);
					printf("APP%d batch%d GEMM_OUTPUT1 cycle all= %ld\n", app_num, batch, pe_cycle);
				}
			} else {
					__raw_writel(0x1,MICC_CYCLE_COUNT0);
					DMA_Transfer_output(
					MEM_GEMM_OUTPUT1_ADDR + GEMM_OUTPUT1_ddrStartAddr_rtl[app_num][0] + SPM_RST_DDR_ADDR,
					MEM_GEMM_OUTPUT1_ADDR + GEMM_OUTPUT1_ddrStartAddr_rtl[app_num][1] + SPM_RST_DDR_ADDR,
					...
					pe_cycle = __raw_readl(MICC_CYCLE_COUNT1);
				__raw_writel(0x2,MICC_CYCLE_COUNT0);
				printf("APP%d GEMM_OUTPUT1 cycle all= %ld\n", app_num, pe_cycle);
			}
			}
			app_num++;
		}
		...
		for (i = 0; i < app_M * app_N * app_K; i++) {
			__raw_writel(0x1,MICC_CYCLE_COUNT0);
			DMA_Transfer_input(MEM_GEMM_INPUT1_ADDR + GEMM_INPUT1_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR,
								MEM_GEMM_INPUT1_ADDR + GEMM_INPUT1_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR,
								GEMM_INPUT1_spmStartAddr[app_num][0], GEMM_INPUT1_spmStartAddr[app_num][1],
			...
			__raw_writel(0x1,MICC_CYCLE_COUNT0);
			DMA_Transfer_input(MEM_GEMM_INPUT2_ADDR + GEMM_INPUT2_ddrStartAddr_rtl[app_num][0] + SPM_DDR_ADDR,
								MEM_GEMM_INPUT2_ADDR + GEMM_INPUT2_ddrStartAddr_rtl[app_num][1] + SPM_DDR_ADDR,
								GEMM_INPUT2_spmStartAddr[app_num][0], GEMM_INPUT2_spmStartAddr[app_num][1],
			...
			printf("kernel start : app %d\n", app_num);
			int inst_reload = app_num > 0 ? 0 : 1;
			DPU_Kernel_Start_noneed(inst_reload, TASK_NUM,
